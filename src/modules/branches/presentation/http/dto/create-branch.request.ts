...
import { IsEmail, IsEnum, IsNumber, IsOptional, IsString, IsUUID, Length, Max, Min } from 'class-validator';
...
export class CreateBranchRequest {
  ...
  @ApiProperty({ description: 'Branch name', example: 'Frankfurt Central Branch' })
  @IsString()
  @Length(1, 150)
  name!: string;
  ...
  @ApiProperty({ description: 'Branch status', enum: BranchStatus, required: false, default: BranchStatus.ACTIVE })
  @IsOptional()
  @IsEnum(BranchStatus)
  status?: BranchStatus;
}
