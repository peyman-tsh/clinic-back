...
export class ServiceResponse {
  ...
  @ApiProperty({
    description: 'Buffer time before treatment in minutes',
    example: 5,
  })
  bufferBeforeMinutes!: number;

  @ApiProperty({
    description: 'Buffer time after treatment in minutes',
    example: 10,
  })
  ...
  @ApiProperty({
    description:
      'Total occupied calendar time in minutes (bufferBefore + duration + bufferAfter)',
    example: 45,
  })
  totalOccupiedMinutes!: number;

  @ApiProperty({ description: 'Service price', example: 180.0 })
  price!: number;
